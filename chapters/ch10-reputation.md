...
**Time:** 25 minutes
**Outcome:** A live, automated review request workflow that sends a warm, personalized SMS review request to every completed customer at the 48-hour mark, with a 5-day follow-up for non-responders.

**Prerequisites:** Google Business Profile connected to GHL (Section 10.4). A provisioned GHL phone number with A2P registration (Chapter 5).

---

**Step 1 — Connect Google Business Profile**

If not already connected: **Reputation → Settings → Integrations → Google Business Profile → Connect Google Account**. Authorize and select your location. Confirm the Reviews tab shows your existing reviews.

**Step 2 — Create Your Review Request Template**

Navigate to **Reputation → Requests → + New Request Template**.

Configure:
- **Template Name:** "Post-Service Review Request — SMS"
- **Channel:** SMS
- **Message:** Write a warm, specific message using merge fields. Example:

> "Hi {{contact.first_name}}, it was great working with you! If you have 60 seconds, we'd love your feedback — it helps other clients find us and means a lot to our team. Leave us a quick review here: {{review.link}}"

- **Review Platform:** Select Google
- Save the template
...
Name it: `Review Request — Post-Service Automation`

**Trigger:** Opportunity Stage Changed
- Filter: Pipeline = [Your Pipeline Name] + New Stage = "Completed" (or "Closed Won")

**Action 1 — Wait 48 Hours**
Add a Wait step: 2 days. Configure "Send only during business hours" to avoid sending at 3 AM.

**Action 2 — If/Else: Has complained?**
Condition: Contact does NOT have tag `complaint-active`
- YES (clean): proceed
- NO (has complaint): End Workflow

**Action 3 — Send Review Request**
Add a **Send Review Request** action (found under the Communication actions or the Reputation section of the action picker). Select your "Post-Service Review Request — SMS" template.

**Action 4 — Apply Tag**
Add Tag: `review-request-sent`

**Action 5 — Wait 5 Days**

**Action 6 — If/Else: Review received?**
Condition: Contact does NOT have tag `review-received`
- YES (no review yet): send follow-up
- NO (review received): End Workflow

**Action 7 — Send Follow-Up Request**
Send a second, gentler SMS. Example: "Hi {{contact.first_name}}, just following up in case my earlier message got buried! If you have a moment to leave us a Google review, here's the link: {{review.link}} — Thank you so much!"

**Action 8 — End Workflow**

**Step 4 — Test**

Move a test contact's opportunity to your "Completed" stage manually. Wait for the workflow to trigger (or manually activate for testing). Confirm: Did the SMS send? Does it appear in the contact's activity timeline? Did the tag apply?

**Step 5 — Publish**

Toggle the workflow to Active. From now on, every completed customer receives a perfectly timed, warm review request — without anyone remembering to send it.

:::{note}
**Checklist**
- [ ] Google Business Profile connected in Reputation → Settings
- [ ] Review request SMS template created with merge fields
- [ ] Workflow triggers correctly on opportunity stage change
- [ ] 48-hour wait configured with business-hours restriction
- [ ] Complaint gate If/Else prevents review requests to unhappy customers
- [ ] 5-day follow-up fires only if no review received
- [ ] Test contact received SMS review request
:::
